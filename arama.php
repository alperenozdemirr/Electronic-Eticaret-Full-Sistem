<?php 
  ...
  $name=$_POST['name'];
  if(empty($name)){
    header("Location:index.php?arama=null");
    exit;
  }
$urunler=$db->query("SELECT * FROM urunler WHERE urun_adi like '%$name%' ORDER BY id DESC",PDO::FETCH_OBJ)->fetchAll();
$say=count($urunler);
?>
<head>
    <title>Sonuç</title>
            ...
            <div class="row">
                <div class="col text-center">
                    <div class="section-title">
                        <h2>Ürünler İçerisinde <u>"<?= $name; ?>"</u> Cümlesi Bulunan (<?=$say?>) Adet Ürün Bulundu</h2>
                    </div>
                </div>
            </div>
